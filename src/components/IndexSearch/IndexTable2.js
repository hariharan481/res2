...
import Codedet from "../TabularSearch/Codedetails";
...
const IndexTables1 = () => {
  ...
  const [activeCodeClicked, setActiveCodeClicked] = useState("");
  ...
  const handleCodeClick = async (code) => {
    setClickedCode(code);
    const codeDetails = await fetchCodeDetails(code);
    setResult1(codeDetails);
    setActiveCodeClicked(true); // Set the flag to true when a code is clicked
    global.intable = null;
    setValue(true);
  };
